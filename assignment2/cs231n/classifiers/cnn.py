...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        #TODO check if they fit
        H_out = 1 + (H + 2 * pad - filter_size) // stride
        W_out = 1 + (W + 2 * pad - filter_size) // stride

        self.params['W2'] = np.random.normal(0, weight_scale, size = (num_filters*(H_out//2)*(W_out//2), hidden_dim))
    ...
    def loss(self, X, y=None):
        ...
        #reshape the activation from convolution layer to fit to affine
        A1_reshaped = A1.reshape(N,-1)

        A2, cache2 = affine_relu_forward(A1_reshaped, W2, b2)
